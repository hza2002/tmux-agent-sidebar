...
creation, layout-preserving reuse, and cleanup. `toggle::move_sidebar` owns
cross-window `swap-pane` orchestration and rollback. Automatic following remains
client-aware and may recover an enabled missing live pane, but never enables a
sidebar on its own.
...
- existing slot geometry is authoritative; configured width and position apply
  only when creating a missing slot;
